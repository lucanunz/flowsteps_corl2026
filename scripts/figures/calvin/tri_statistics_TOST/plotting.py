...
def plot_forest(
    ...
    *,
    ...
    x_label: str = "Δ = reference − 1-step (pp)",
    band_label: str | None = None,
) -> None:
    """Forest plot of paired Δ̂ ± 90% CI against a ±delta_pp equivalence band.

    `delta_pp` and `value_scale` are in *display* units: SR@5 uses pp (scale 100,
    δ in pp); the chain-length metric uses subtasks (scale 1, δ in subtasks).
    """
    if not rows:
        return
    n = len(rows)
    ...
    band = band_label or f"±{delta_pp:g} equivalence band"
    ...
    for y, (label, tost) in zip(ys, rows):
        if tost is None:
            ...
            continue
        d = tost.delta_hat * value_scale
        lo, hi = tost.ci_low * value_scale, tost.ci_high * value_scale
